...
const buttonStyles = {
  buttonWrapper:
    'AlignmentToolButtonWrapper',
  button: 'AlignmentToolButton',
  active: 'AlignmentToolButton_active'
}
...
export default class AlignmentTool extends React.Component {
  ...
  onVisibilityChanged = visibleBlock => {
    setTimeout(() => {
      let position
      const boundingRect = this.props.store.getItem(
        'boundingRect'
      )
      if (
        visibleBlock &&
        ...
        {defaultButtons.map(
          (Button, index) => (
            <Button
              ...
              setAlignment={this.props.store.getItem(
                'setAlignment'
              )}
              theme={buttonStyles}
            />
