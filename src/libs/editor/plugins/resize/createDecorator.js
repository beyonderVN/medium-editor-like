...
export default ({
  ...
  class BlockResizeableDecorator extends Component {
    static displayName = `Resizable(${getDisplayName(
      WrappedComponent
    )})`
    ...
    setEntityData = data => {
      this.props.blockProps.setResizeData(
        data
      )
    }
    ...
    mouseMove = evt => {
      ...
      const pane = ReactDOM.findDOMNode(
        this
      )
      ...
      const isTop =
        vertical && vertical !== 'auto'
          ? y < tolerance
          : false
      const isLeft = horizontal
        ? x < tolerance
        : false
      const isRight = horizontal
        ? x >= b.width - tolerance
        : false
      const isBottom =
        vertical && vertical !== 'auto'
          ? y >= b.height - tolerance &&
            y < b.height
          : false
      ...
      const hasNewHoverPositions = Object.keys(
        newHoverPosition
      ).filter(
        key =>
          hoverPosition[key] !==
          newHoverPosition[key]
      )
